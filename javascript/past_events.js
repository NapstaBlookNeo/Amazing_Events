...
for (let i = 0; i < data.events.length; i++) {
        let eventDate = new Date(data.events[i].date)
            if (eventDate < todayDate) {
                pastEventsdata[tempArrayPosition] = data.events[i]
                tempArrayPosition++
            }   
}
...
for (let i = 0; i < pastEventsdata.length; i += 4) {
    let carruselItem
    if (i < 4) {
    ...
    for (let j = i; j < i + 4; j++) {
        if (pastEventsdata[j] != undefined) {
            let card = document.createElement("div")
            card.classList.add("card", "mx-1", "my-3", "border", "col-12", "col-md-3", "cardSize")
            card.innerHTML = `
            <img src="${pastEventsdata[j].image}" class="card-img-top h-50" alt="${pastEventsdata[j].name}">
            <div class="card-body">
                <h5 class="card-title">${pastEventsdata[j].name}</h5>
                <p class="card-text">${pastEventsdata[j].description}</p>
            </div>
            <div class="card-text d-flex justify-content-around align-items-center">
            <p>Price: ${pastEventsdata[j].price}$</p>
